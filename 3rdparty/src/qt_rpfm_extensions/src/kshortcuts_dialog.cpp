...
void new_action(QObject* actions, QString const action_name, QString const display_name, Qt::ShortcutContext context, QList<QKeySequence> shortcut, QString const icon_name = "") {
    QAction* action = new QAction(actions);
...
QObject* new_collection(QWidget* parent, QString const group_name, QString const display_name) {
    QObject* collection = new QObject(parent);
    collection->setObjectName(group_name);
    collection->setProperty("displayName", display_name);
    return collection;
}

extern "C" void shortcut_collection_init(QWidget* parent, QList<QObject*>* shortcuts) {
    QObject* pack_menu_actions = new_collection(parent, "pack_menu", "Pack Menu");
    ...
    new_action(pack_menu_actions, "save_current", "Save Current Pack", Qt::ShortcutContext::ApplicationShortcut, QKeySequence::listFromString("Ctrl+Shift+S"), "document-save");
    ...
    QObject* mymod_menu_actions = new_collection(parent, "mymod_menu", "MyMod Menu");
    ...
    QObject* view_menu_actions = new_collection(parent, "view_menu", "View Menu");
    ...
    QObject* command_palette_actions = new_collection(parent, "command_palette", "Command Palette");
    ...
    QObject* game_selected_menu_actions = new_collection(parent, "game_selected_menu", "Game Selected Menu");
    ...
    QObject* about_menu_actions = new_collection(parent, "about_menu", "About Menu");
    ...
    QObject* file_tab_actions = new_collection(parent, "file_tab", "File Tabs");
    ...
    QObject* pack_tree_actions = new_collection(parent, "pack_tree_context_menu", "Pack Tree Context Menu");
    ...
    QObject* dependencies_tree_actions = new_collection(parent, "dependencies_context_menu", "Dependencies Tree Context Menu");
    ...
    QObject* diagnostics_table_actions = new_collection(parent, "diagnostics_context_menu", "Diagnostics Table Context Menu");
    ...
    QObject* anim_pack_tree_actions = new_collection(parent, "anim_pack_tree_context_menu", "AnimPack Tree Context Menu");
    ...
    QObject* secondary_pack_tree_actions = new_collection(parent, "secondary_pack_tree_context_menu", "Pack Tree Context Menu");
    ...
    QObject* table_editor_actions = new_collection(parent, "table_editor", "Table Editor");
    ...
    new_action(table_editor_actions, "go_to_file", "Go To File", Qt::ShortcutContext::WidgetShortcut, QKeySequence::listFromString(""), "go-jump-definition");
    ...
    QObject* decoder_actions = new_collection(parent, "decoder", "Decoder");
    ...
    QObject* portrait_settings_actions = new_collection(parent, "portrait_settings", "Portrait Settings");
    ...
    QObject* unit_variant_actions = new_collection(parent, "unit_variant", "Unit Variant");
    ...
    QObject* translator_actions = new_collection(parent, "translator", "Translator");
    ...
    QObject* text_editor_actions = new_collection(parent, "text_editor", "Text Editor");

    shortcuts->append(pack_menu_actions);
    shortcuts->append(mymod_menu_actions);
    shortcuts->append(view_menu_actions);
    shortcuts->append(game_selected_menu_actions);
    shortcuts->append(about_menu_actions);
    shortcuts->append(file_tab_actions);
    shortcuts->append(pack_tree_actions);
    shortcuts->append(dependencies_tree_actions);
    shortcuts->append(diagnostics_table_actions);
    shortcuts->append(anim_pack_tree_actions);
    shortcuts->append(secondary_pack_tree_actions);
    shortcuts->append(table_editor_actions);
    shortcuts->append(decoder_actions);
    shortcuts->append(portrait_settings_actions);
    shortcuts->append(unit_variant_actions);
    shortcuts->append(text_editor_actions);
    shortcuts->append(translator_actions);
    shortcuts->append(command_palette_actions);
}
...
extern "C" QAction* shortcut_action(QList<QObject*> const &shortcuts, QString const action_group, QString const action_name) {
    for (QObject* collection : shortcuts) {
        if (collection->objectName() == action_group) {
            QAction* action = collection->findChild<QAction*>(action_name, Qt::FindDirectChildrenOnly);
            if (action != nullptr) {
...
extern "C" void kshortcut_dialog_init(QWidget* widget, QList<QObject*>* shortcuts) {
    Q_UNUSED(shortcuts);
    QMessageBox::information(widget, QStringLiteral("Shortcuts"), QStringLiteral("Shortcut editing requires KDE Frameworks and is disabled in this macOS Qt-only build."));
}
