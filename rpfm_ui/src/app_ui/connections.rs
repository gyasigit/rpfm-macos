...
use crate::ffi::{flush_pending_open_packs_safe, main_window_drop_pack_signal, main_window_theme_changed_signal};
...
pub unsafe fn set_connections(app_ui: &Rc<AppUI>, slots: &AppUISlots) {
    ...
    app_ui.tools_ceo_builder.triggered().connect(&slots.tools_ceo_builder);
    ...
    flush_pending_open_packs_safe(&app_ui.main_window);
    ...
    app_ui.welcome_page_ui().feedback_button().released().connect(&slots.send_feedback);
}
